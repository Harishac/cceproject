...
if(k == 0 ):
    ...
    for i in df.index:        
        a1 = a1 + (df.loc[i][col_name_x]**3 )                                  #sum(x)
        c1 = c1 + df.loc[i][col_name_y]                                   #sum(y) 
        a2 = a2 + (df.loc[i][col_name_x]**6)         #sum(x**4)
        c2 = c2 + ((df.loc[i][col_name_x]**3) * df.loc[i][col_name_y])         #sum(x^2.y)
    ...
    df['ycal'] = [(m*(df.loc[i][col_name_x]**3)) + c for i in df.index]
    #print df
...
#    print 'sqr(y-ycap)', sse_list
    ...
    for i in df.index:        
        a1 = a1 + (df.loc[i][col_name_x1]**3)                               #sum(x1)
        b1 = b1 + (df.loc[i][col_name_x2]**3)                                   #sum(x2)
        d1 = d1 + df.loc[i][col_name_y]                                    #sum(y) 
        a2 = a2 + (df.loc[i][col_name_x1]**6)        #sum(x1 square)
        b2 = b2 + ((df.loc[i][col_name_x1]**3) * (df.loc[i][col_name_x2]**3))        #sum(x1.x2) 
        d2 = d2 + ((df.loc[i][col_name_x1]**3) * df.loc[i][col_name_y])         #sum(x1.y)
        b3 = b3 + (df.loc[i][col_name_x2]**6)        #sum(x2 square)
        d3 = d3 + ((df.loc[i][col_name_x2]**3) * df.loc[i][col_name_y])         #sum(x2.y)
    ...
    df['ycal'] = [(m1*(df.loc[i][col_name_x1]**3)) + (m2*(df.loc[i][col_name_x2]**3)) + c for i in df.index]
    #print df
...
#    print 'sqr(y-ycap)', sse_list
    ...
    for i in df.index:        
        a1 = a1 + (df.loc[i][col_name_x1]**3)                                  #sum(x1)
        b1 = b1 + (df.loc[i][col_name_x2]**3)                                   #sum(x2)
        c1 = c1 + (df.loc[i][col_name_x3]**3)                                   #sum(x3)
        e1 = e1 + df.loc[i][col_name_y]                                    #sum(y)
        
        a2 = a2 + (df.loc[i][col_name_x1]**6)        #sum(x1 square)
        b2 = b2 + ((df.loc[i][col_name_x1]**3) * (df.loc[i][col_name_x2]**3))        #sum(x1.x2) 
        c2 = c2 + ((df.loc[i][col_name_x1]**3) * (df.loc[i][col_name_x3]**3))        #sum(x1.x3) 
        e2 = e2 + ((df.loc[i][col_name_x1]**3) * df.loc[i][col_name_y])         #sum(x1.y)

        b3 = b3 + (df.loc[i][col_name_x2]**6)        #sum(x2 square)
        c3 = c3 + ((df.loc[i][col_name_x2]**3) * (df.loc[i][col_name_x3]**3))        #sum(x2.x3) 
        e3 = e3 + ((df.loc[i][col_name_x2]**3) * df.loc[i][col_name_y])         #sum(x2.y)

        c4 = c4 + (df.loc[i][col_name_x3]**6)        #sum(x3 square) 
        e4 = e4 + ((df.loc[i][col_name_x3]**3) * df.loc[i][col_name_y])         #sum(x3.y)
    ...
    df['ycal'] = [(m1*(df.loc[i][col_name_x1]**3)) + (m2*(df.loc[i][col_name_x2]**3)) + (m3*(df.loc[i][col_name_x3]**3)) + c for i in df.index]
    #print df
